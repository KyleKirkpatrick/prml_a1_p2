...
def plot_predictions(x_test, y_test, y_pred, indices, figure_title, filename, title_colour=None):
    plt.figure(figsize=(10, 4))
    for position, index in enumerate(indices[:10]):
        plt.subplot(2, 5, position + 1)
        plt.imshow(x_test[index], cmap="gray")
        image_title = f"Pred: {CLASS_NAMES[y_pred[index]]}\nTrue: {CLASS_NAMES[y_test[index]]}"
        if title_colour is None:
            plt.title(image_title, fontsize=8)
        else:
            plt.title(image_title, color=title_colour, fontsize=8)
        plt.axis("off")
